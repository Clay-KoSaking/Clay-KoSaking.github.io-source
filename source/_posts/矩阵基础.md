...
- **对称矩阵**：满足 $\boldsymbol{A}^\top=\boldsymbol{A}$ 的**方阵** $\boldsymbol{A}$．
- **反对称矩阵**：满足 $\boldsymbol{A}^\top=-\boldsymbol{A}$ 的**方阵** $\boldsymbol{A}$．
...
### 矩阵转置
矩阵转置的性质如下所示．
1. **二次转置**：$(\boldsymbol{A}^\top)^\top=\boldsymbol{A}$．
2. **相加后转置**：$(\boldsymbol{A}+\boldsymbol{B})^\top=\boldsymbol{A}^\top+\boldsymbol{B}^\top$．
3. **数乘后转置**：$(k\boldsymbol{A})^\top=k\boldsymbol{A}^\top$．
4. **相乘后转置**：$(\boldsymbol{AB})^\top=\boldsymbol{B}^\top\boldsymbol{A}^\top$．
...
行列式的性质如下所示．
1. 方阵转置后，行列式的值不变，即 $|\\,\boldsymbol{A}\\,|=|\\,\boldsymbol{A}^\top\\,|$．
...
2. 若矩阵 $\boldsymbol{A}$ 可逆，则 $\boldsymbol{A}^\top$ 同样可逆，且 $(\boldsymbol{A}^\top)^{-1}=(\boldsymbol{A}^{-1})^\top$．
...
\boldsymbol{A}^\top=\left(\\;\begin{matrix}
    \boldsymbol{A} _{11}^\top & \boldsymbol{A} _{21}^\top & \cdots & \boldsymbol{A} _{m1}^\top \\\\
    \boldsymbol{A} _{12}^\top & \boldsymbol{A} _{22}^\top & \cdots & \boldsymbol{A} _{m2}^\top \\\\
    \vdots                 & \vdots                 &        & \vdots                 \\\\
    \boldsymbol{A} _{1n}^\top & \boldsymbol{A} _{2n}^\top & \cdots & \boldsymbol{A} _{mn}^\top
\end{matrix}\\;\right)
$$
