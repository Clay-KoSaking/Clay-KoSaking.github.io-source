...
6. $n$ 个 $n$ 维向量**线性无关**的**充要条件**为其组成的行列式的值 $D\neq 0$．
    ...
    <div style="margin-top: .8rem">本质是克莱姆法则的应用．</div>
    </div>
...
### 正交矩阵
若 $n$ 阶方阵 $\boldsymbol{A}$ 满足 $\boldsymbol{A}^\top\boldsymbol{A}=\boldsymbol{E}$，则称 $\boldsymbol{A}$ 为 $n$ 阶**正交矩阵**．

正交矩阵的性质如下所示．
1. 对于正交矩阵 $\boldsymbol{A}$ 而言，有 $\boldsymbol{A}^\top=\boldsymbol{A}^{-1}$．
2. 若 $\boldsymbol{A}$ 为正交矩阵，则 $|\\,\boldsymbol{A}\\,|=\pm 1$．
3. 若 $\boldsymbol{A}$ 为正交矩阵，则 $\boldsymbol{A}^\top$ 与 $\boldsymbol{A}^{-1}$ 均为正交矩阵．
...
1. 方阵 $\boldsymbol{A}$ 与其转置 $\boldsymbol{A}^\top$ 有相同的特征值．

    <div class="admonition warning">
    <div class="admonition-title">证明</div>
    <div style="margin-top: .8rem">由于 $|\;\boldsymbol{A}^\top-\lambda\boldsymbol{E}\;|=|\;\boldsymbol{A}^\top-\lambda\boldsymbol{E}^\top\;|=|\;(\boldsymbol{A}-\lambda\boldsymbol{E})^\top\;|=|\;\boldsymbol{A}-\lambda\boldsymbol{E}\;|$，故 $\boldsymbol{A}$ 与 $\boldsymbol{A}^\top$ 在特征方程上同解，证毕．</div>
    </div>

    <div class="admonition error">
    <div class="admonition-title">注意事项</div>
    <div style="margin-top: .8rem">方阵 $\boldsymbol{A}$ 与其转置 $\boldsymbol{A}^\top$ 对应的特征向量<b>不一定相同</b>．</div>
    </div>
